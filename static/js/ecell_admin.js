...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function createUpdateDetailCard() {
        ...
        // The E-Cell HTML already contains the modal markup.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function identityEditor() {
    openModal("CLUB IDENTITY", "Edit E-Cell Club");
  ...
  function bindUpdate(el) {
    ...
    // NEW: dynamically created updates also open the existing preview card.
    if (!el.dataset.previewBound) {
      el.dataset.previewBound = "true";
      el.addEventListener("click", function (e) {
        if (e.target.closest(".admin-update-pencil")) return;

        const overlay = document.getElementById("updateModalOverlay");
        if (!overlay) return;

        const imageWrap = document.getElementById("updateModalImageWrap");
        const imageEl = document.getElementById("updateModalImage");
        const categoryEl = document.getElementById("updateModalCategory");
        const titleEl = document.getElementById("updateModalTitle");
        const uploaderEl = document.getElementById("updateModalUploader");
        const dateEl = document.getElementById("updateModalDate");
        const deadlineRow = document.getElementById("updateModalDeadlineRow");
        const deadlineEl = document.getElementById("updateModalDeadline");
        const descEl = document.getElementById("updateModalDesc");
        const contactRow = document.getElementById("updateModalContactRow");
        const emailLink = document.getElementById("updateModalEmail");
        const emailText = document.getElementById("updateModalEmailText");
        const applyBtn = document.getElementById("updateModalApply");

        const data = {
          title: el.dataset.title || "",
          image: el.dataset.image || "",
          uploadDate: el.dataset.uploadDate || "",
          uploadTime: el.dataset.uploadTime || "",
          uploadedBy: el.dataset.uploadedBy || "",
          deadline: el.dataset.deadline || "",
          category: el.dataset.category || "",
          categoryColor: el.dataset.categoryColor || "",
        ...
        titleEl.textContent = data.title || "Untitled Update";
        if (data.category) {
        ...
        uploaderEl.textContent = data.uploadedBy || "—";
        const dateParts = [];
        if (data.uploadDate) dateParts.push(data.uploadDate);
        if (data.uploadTime) dateParts.push(data.uploadTime);
        dateEl.textContent = dateParts.length ? dateParts.join(" · ") : "—";

        if (data.deadline) {
          deadlineEl.textContent = "Deadline: " + data.deadline;
          deadlineRow.classList.remove("hidden");
        } else deadlineRow.classList.add("hidden");
  ...
  function contactEditor() {
    const items = $$(".ecell-contact-item");
...
document.addEventListener("DOMContentLoaded", () => {
  const data = {
    complaints: [
      ...
      {name:"Anjali Devi", email:"anjali23@rgukt.ac.in", branch:"ECE", year:"2nd Year", phone:"+91 98765 10002", subject:"Suggestion for E-Cell session", category:"Suggestion", message:"Please conduct more hands-on sessions on E-Cell IAM, VPC and cloud security.", submitted:"Yesterday, 6:15 PM", status:"Pending"},
    ...
    applications: [
      {name:"Sai Teja", email:"saiteja@rgukt.ac.in", branch:"CSE", year:"3rd Year", phone:"+91 98765 20001", event:"E-Cell Cloud Practitioner Workshop", registrationId:"E-Cell-2026-1042", skills:"Python, Linux, E-Cell basics", motivation:"I want to prepare for the E-Cell Cloud Practitioner certification.", submitted:"Today, 9:10 AM", status:"Pending"},
      {name:"Meghana", email:"meghana@rgukt.ac.in", branch:"ECE", year:"2nd Year", phone:"+91 98765 20002", event:"Cloud Hackathon 2026", registrationId:"E-Cell-2026-1043", skills:"Java, Web Development", motivation:"I want to build a cloud-based campus solution.", submitted:"Today, 8:55 AM", status:"Pending"},
      {name:"Harsha", email:"harsha@rgukt.ac.in", branch:"CSE", year:"1st Year", phone:"+91 98765 20003", event:"E-Cell Cloud Practitioner Workshop", registrationId:"E-Cell-2026-1044", skills:"C, Python", motivation:"Interested in starting my cloud journey.", submitted:"Yesterday, 7:40 PM", status:"Pending"},
      {name:"Divya", email:"divya@rgukt.ac.in", branch:"EEE", year:"2nd Year", phone:"+91 98765 20004", event:"Cloud Hackathon 2026", registrationId:"E-Cell-2026-1045", skills:"UI/UX, Figma", motivation:"I would like to contribute as a designer and learn cloud deployment.", submitted:"Yesterday, 4:30 PM", status:"Pending"}
    ],
    members: [
      {name:"Akash", email:"akash@rgukt.ac.in", branch:"CSE", year:"2nd Year", phone:"+91 98765 30001", rollNumber:"R20240101", interests:"Cloud computing, DevOps", experience:"Completed E-Cell Cloud Quest.", motivation:"I want to actively participate in E-Cell workshops and events.", submitted:"Today, 11:00 AM", status:"Pending"},
      ...
      {name:"Naveen", email:"naveen@rgukt.ac.in", branch:"ME", year:"1st Year", phone:"+91 98765 30003", rollNumber:"R20250121", interests:"E-Cell, Serverless", experience:"Beginner", motivation:"I want to learn through hands-on projects.", submitted:"Yesterday, 5:25 PM", status:"Pending"},
      ...
      {name:"Vishal", email:"vishal@rgukt.ac.in", branch:"EEE", year:"3rd Year", phone:"+91 98765 30005", rollNumber:"R20230318", interests:"Cloud architecture", experience:"E-Cell Educate learner.", motivation:"Looking for a community to learn and collaborate.", submitted:"17 Aug, 7:40 PM", status:"Pending"}
    ]
  };
  ...
  const meta = {
    ...
    members:{title:"New Club Member Registrations", desc:"Review students who submitted the E-Cell join form.", eyebrow:"MEMBER MANAGEMENT"}
  };
  ...
  list.addEventListener("click", e => {
    ...
    detail.innerHTML = `
      <div class="detail-top">
        <div>
          <span class="ecell-admin-eyebrow">${type === "complaints" ? "COMPLAINT DETAILS" : type === "applications" ? "APPLICATION DETAILS" : "MEMBER REGISTRATION"}</span>
          <h3>${item.name}</h3>
  ...
  document.getElementById("ecellAdminProfileBtn").addEventListener("click",openSide);
  ...
  function showToast(message){const t=document.getElementById("ecellAdminToast");t.textContent=message;t.classList.add("show");setTimeout(()=>t.classList.remove("show"),2800)}
});
