...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function identityEditor() {
    openModal("CLUB IDENTITY", "Edit Artix Club");
  ...
  function bindUpdate(el) {
    ...
    // Keep the existing pencil/edit behavior.
    ...
    // Newly created updates are added after artix.js attaches click
    // listeners to the hardcoded updates. Attach the same preview action
    // here so new updates open the existing student-style preview card.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadClubComplaints() {
    try {
      ...
      data.complaints = (result.complaints || []).map(complaint => ({
        ...
        status: {new:"Pending", "under-review":"Under Review", "in-progress":"In Progress", resolved:"Resolved", rejected:"Rejected"}[complaint.status] || complaint.status
      }));
    } catch (error) {
      console.error("Could not load club complaints:", error);
    }
  }
  ...
  const meta = {
    ...
    members:{title:"New Club Member Registrations", desc:"Review students who submitted the Artix join form.", eyebrow:"MEMBER MANAGEMENT"}
  };
  ...
  list.addEventListener("click", e => {
    ...
    detail.innerHTML = `
      <div class="detail-top">
        <div>
          <span class="aws-admin-eyebrow">${type === "complaints" ? "COMPLAINT DETAILS" : type === "applications" ? "APPLICATION DETAILS" : "MEMBER REGISTRATION"}</span>
          <h3>${item.name}</h3>
    ...
    document.getElementById("saveSubmissionStatus").addEventListener("click", async e => {
      ...
      if (btn.dataset.type === "complaints") {
        try {
          const response = await fetch(`/complaints/api/${encodeURIComponent(submission.referenceId)}/status`, {method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({status:newStatus})});
          const result = await response.json();
          if (!response.ok || !result.success) throw new Error(result.message || "Could not update complaint status.");
        } catch (error) {
  ...
  document.getElementById("awsAdminProfileBtn").addEventListener("click",openSide);
